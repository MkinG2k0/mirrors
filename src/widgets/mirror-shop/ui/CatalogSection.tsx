...
interface CatalogSectionProps {
  ...
  onSelectMirrorForCalc: (index: number) => void;
  className?: string;
}
...
export function CatalogSection({
  ...
  onCategoryChange,
  onNavigate,
  onSelectMirrorForCalc,
  className,
}: CatalogSectionProps) {
  const filtered =
    category === "all" ? MIRRORS : MIRRORS.filter((m: Mirror) => m.category === category);
          ...
          <button
            ...
            className={cn(
              ...
              category === c.key
                ...
                : "border border-white/[0.08] text-text-dim hover:text-gold"
            )}
          >
            {c.label}
          </button>
      ...
      <div className="grid gap-6 [grid-template-columns:repeat(auto-fill,minmax(320px,1fr))]">
        {filtered.map((m) => (
          <article key={m.id} className={cardBase}>
            <div className="mb-5 flex h-40 items-center justify-center border-b border-white/5 bg-gradient-to-br from-surface-2 to-gold/5 text-5xl">
              {m.image}
            </div>
            <div className="mb-3 flex items-start justify-between">
              <h3 className="font-cormorant text-xl font-normal text-text-primary">{m.name}</h3>
              <span className={tagBase}>{m.thickness}</span>
            </div>
            <p className="font-outfit mb-4 text-[13px] leading-snug text-text-dim">{m.desc}</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                <span className="font-outfit text-[11px] text-text-dim">от </span>
              ...
              <button
                type="button"
                onClick={() => {
                  onSelectMirrorForCalc(MIRRORS.indexOf(m));
